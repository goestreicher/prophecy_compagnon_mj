...
class EquipmentModelListFilterWidget extends StatefulWidget {
  const EquipmentModelListFilterWidget({
    super.key,
    required this.onFilterChanged,
    this.filter,
  });

  final void Function(EquipmentModelListFilter) onFilterChanged;
  final EquipmentModelListFilter? filter;

  @override
  State<EquipmentModelListFilterWidget> createState() => _EquipmentModelListFilterWidgetState();
}

class _EquipmentModelListFilterWidgetState extends State<EquipmentModelListFilterWidget> {
  final TextEditingController sourceTypeController = TextEditingController();
  final TextEditingController sourceController = TextEditingController();
  final TextEditingController searchController = TextEditingController();

  late EquipmentModelListFilter currentFilter;

  @override
  void initState() {
    super.initState();

    currentFilter = widget.filter ?? EquipmentModelListFilter();
    ...
    return Wrap(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            Switch(
              value: currentFilter.unique,
              onChanged: (bool v) {
                setState(() {
                  currentFilter.unique = v;
                });
                widget.onFilterChanged(currentFilter);
              },
            ),
            Text('Équipements uniques'),
          ],
        ),
        SizedBox(
          width: 200.0,
